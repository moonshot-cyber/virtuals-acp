...
export async function executeJob(request: any): Promise<ExecuteJobResult> {
  ...
  const res = await fetch(`${API_BASE}/counterparties/${address}`, {
...
export function requestPayment(request: any): string {
  return `Counterparty analysis for ${request.wallet_address}. Fetching top interactions enriched with Nansen labels.`;
}
