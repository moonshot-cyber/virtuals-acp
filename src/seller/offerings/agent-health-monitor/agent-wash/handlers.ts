...
export async function executeJob(request: any): Promise<ExecuteJobResult> {
  ...
  const res = await fetch(`${API_BASE}/wash/${address}`, {
    method: "POST",
...
export function requestPayment(request: any): string {
  return `Wallet hygiene scan for ${request.wallet_address}. Checking dust tokens, spam, gas efficiency, and failed tx patterns.`;
}
