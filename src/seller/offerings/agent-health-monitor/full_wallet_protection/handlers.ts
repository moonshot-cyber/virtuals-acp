...
export async function executeJob(request: any): Promise<ExecuteJobResult> {
  ...
  const res = await fetch(`${API_BASE}/agent/protect/${address}`, {
...
export function requestPayment(request: any): string {
  return `Full wallet protection for ${request.wallet_address}. Running health, gas optimization, and retry analysis.`;
}
