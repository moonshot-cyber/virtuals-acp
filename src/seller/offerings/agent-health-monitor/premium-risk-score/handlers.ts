...
export async function executeJob(request: any): Promise<ExecuteJobResult> {
  ...
  const res = await fetch(`${API_BASE}/risk/premium/${address}`, {
...
export function requestPayment(request: any): string {
  return `Premium risk assessment for ${request.wallet_address}. Analyzing risk score and fetching Nansen wallet intelligence labels.`;
}
