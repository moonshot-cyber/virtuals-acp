...
export async function executeJob(request: any): Promise<ExecuteJobResult> {
  ...
  const res = await fetch(`${API_BASE}/network-map/${address}?chain=${chain}`, {
...
export function requestPayment(request: any): string {
  const chain = request.chain || "ethereum";
  return `Wallet network map for ${request.wallet_address} on ${chain}. Fetching related wallets via Nansen.`;
}
