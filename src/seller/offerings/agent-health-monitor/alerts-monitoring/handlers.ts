...
export async function executeJob(request: any): Promise<ExecuteJobResult> {
  ...
  const res = await fetch(`${API_BASE}/alerts/subscribe/${address}`, {
...
export function requestPayment(request: any): string {
  return `Setting up alerts monitoring for ${request.wallet_address}. Will check wallet health every 6 hours and send alerts when issues are detected.`;
}
